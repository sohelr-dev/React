...
function UpdateRole() {
    ...
    // console.log(paramId)
    const getRoleId=(()=>{
        api.get(`details-role?id=${paramId}`)
        .then((res)=>{
                        ...
                        <div className="card mt-3 shadow-sm">
                            ...
                            <div className="card-body">
                                <form onSubmit={handleSubmit}>
                                        <input type="hidden" name="id" id="id" value={roleId.id} onChange={(e) => setRoleId({ ...roleId, id: parseInt(e.target.value) })}/>
                                    <div className="mb-3">
                                        <label htmlFor="name" className="form-label mb-4"> Role </label>
                                        <input type="text" id="role_name" name="role_name" className="form-control" placeholder="Enter role Name"
                                            value={roleId.role_name} onChange={(e) => setRoleId({ ...roleId, role_name: e.target.value })}
                                            required />
                                    </div>
