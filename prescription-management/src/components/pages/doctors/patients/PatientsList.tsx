...
function PatientsList() {
  ...
  const handleDelete = (deleteId: any) => {
    api.delete(`delete-patient?id=${deleteId}`)
      .then((response) => {
  ...
  return (
    <>
      ...
      <Link to="/patients/create-patient" className="btn btn-primary mb-4">Create </Link>
              ...
              <tbody>
                {patients.map((item) => (
                  <tr key={item.id} className="text-center">
                    ...
                    <td>{item.name}</td>
                    <td>{item.age ?? ""}</td>
                    <td>{item.gender ? item.gender.charAt(0).toUpperCase() + item.gender.slice(1).toLowerCase() : "N/A"}</td>


                    <td >
                      <div className="d-flex justify-content-center gap-2">
                        <Link to={`/patients/details-patient/${item.id}`} className="btn btn-outline-primary btn-sm">
                          <i className="fas fa-eye"></i>
                        </Link>
                        <Link to={`/patients/edit-patient/${item.id}`} className="btn btn-primary btn-sm" >
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            ...
            <div className="modal-footer">
              ...
              <button type="button" className="btn btn-primary" data-bs-dismiss="modal" onClick={() => handleDelete(patientId)} >Delete</button>
            </div>
          </div>
        </div>
      </div>
